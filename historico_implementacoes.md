...
### [31/08/2026] Etapa 63 Concluída: Métricas nas Pastas da Central de Missões para Vistoriadores, Link Discreto de Limpeza de Cache e Roteamento por Trânsito Viário Real (Carro/OSRM)
- **1. Métricas Operacionais nos Cards de Pastas (`MissionManagerModal.jsx`):**
  - Em cada card de pasta de quartel (`displayFolders`), são plotados diretamente: volume de missões ativas, razão e percentual de hidrantes vistoriados com barra de progresso colorida, e mini-badges de status (`⏳ Plan.`, `🔄 And.`, `✅ Conc.`).
  - Vistoriadores podem fixar o quartel favorito (`★ Favorita`) para mantê-lo no topo e acessar o Dashboard de Comando em modo de visualização geral.
- **2. Estilo Discreto de Atualizar Versão / Limpar Cache (`App.jsx`):**
  - O botão de atualizar sistema / limpar cache foi convertido em um link minimalista e sutil no rodapé do menu suspenso (junto ao crédito do desenvolvedor), deixando de disputar destaque visual com os módulos operacionais militares.
- **3. Roteamento por Trânsito Viário de Veículos (`MissionRoutePanel.jsx`):**
  - Substituída a ordenação euclidiana pura pelo cálculo assíncrono de Matriz Viária de Condução de Carro (`https://router.project-osrm.org/table/v1/driving/` com `annotations=duration,distance`), calculando a sequência de vistorias com base nas vias reais de trânsito do DF.
  - Nos cards dos hidrantes na rota, a distância e o tempo estimado de condução são exibidos com precisão (ex: `1.4 km via trânsito • ~3min`) com o badge indicador `🚗 Vias de Trânsito`.
