...
const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'hidrantes', 'brazlandia');
...
async function runBrazlandia() {
  console.log("==========================================================");
  console.log("🎯 PIPELINE STREET VIEW SNIPER - BRAZLANDIA");
  console.log("==========================================================\n");
  ...
  const targetHydrants = allRows.filter(r => 
    (r.nomHidrante || '').toUpperCase().startsWith('BRZ') ||
    (r.nomHidrante || '').toUpperCase().startsWith('BRA') ||
    (r.cidade || '').toUpperCase().includes('BRAZ') ||
    (r.dscLocalidade || '').toUpperCase().includes('BRAZ')
  );

  console.log(`📍 Processando ${targetHydrants.length} hidrantes em Brazlandia...\n`);
  ...
  for (let i = 0; i < targetHydrants.length; i++) {
    ...
    const relativeUrl = `/hidrantes/brazlandia/${nom}.jpeg`;
  ...
  if (failedHydrants.length > 0) {
    if (!failureLog.includes("## Brazlandia")) {
      failureLog += "\n## Brazlandia\n";
    }
  ...
  console.log(`\n==========================================================`);
  console.log(`🎉 EXTRAÇÃO DE BRAZLANDIA CONCLUÍDA!`);
