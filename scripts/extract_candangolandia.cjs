...
const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'hidrantes', 'candangolandia');
...
async function runCandangolandia() {
  console.log("==========================================================");
  console.log("🎯 PIPELINE STREET VIEW SNIPER - CANDANGOLÂNDIA");
  console.log("==========================================================\n");
  ...
  const targetHydrants = allRows.filter(r => 
    (r.nomHidrante || '').toUpperCase().startsWith('CAN') ||
    (r.cidade || '').toUpperCase().includes('CAND') ||
    (r.dscLocalidade || '').toUpperCase().includes('CAND')
  );

  targetHydrants.sort((a, b) => (a.nomHidrante || '').localeCompare(b.nomHidrante || ''));

  console.log(`📍 Processando ${targetHydrants.length} hidrantes em Candangolândia...\n`);

  let failedHydrants = [];
  let noCoverageHydrants = [];
  const updatedPhotosMap = {};
  let countUpdated = 0;

  for (let i = 0; i < targetHydrants.length; i++) {
    const h = targetHydrants[i];
    const nom = h.nomHidrante || `CAN_${h.codHidrante}`;
    const gpsLat = parseFloat(h.numLatitude);
    const gpsLng = parseFloat(h.numLongitude);

    console.log(`\n🔎 [${i+1}/${targetHydrants.length}] ${nom} (Cód: ${h.codHidrante})`);
    console.log(`   Endereço: ${h.dscEndereco || 'Sem endereço'} | GPS: ${gpsLat}, ${gpsLng}`);

    const fileBase = path.join(OUTPUT_DIR, nom);
    const relativeUrl = `/hidrantes/candangolandia/${nom}.jpeg`;
    ...
    if (fs.existsSync(`${fileBase}.jpeg`) && fs.statSync(`${fileBase}.jpeg`).size > 5000 &&
      ...
      console.log(`   ⚡ Fotos (Padrão e HD) já existem localmente. Pulando download...`);
    ...
    if (!metaRes || metaRes.status !== 'OK') {
      console.log(`   ⚠️ Sem cobertura Street View neste ponto.`);
      noCoverageHydrants.push(nom);
      continue;
    }
    ...
    if (!result.success) {
      console.log(`   🚧 Hidrante não detectado de primeira. Iniciando [Passo 2] Step-Around...`);
      const altMeta = await findAdjacentPanorama(gpsLat, gpsLng, initialPanoId);
      
      if (altMeta) {
        console.log(`   🚶‍♂️ Deslocou com sucesso para panorama adjacente. Refazendo Scan...`);
    ...
    if (!result.success) {
      console.log(`   ⚠️ IA não encontrou com confiança >= 60%. Aplicando FALLBACK para GPS original.`);
  ...
  if (noCoverageHydrants.length > 0 || failedHydrants.length > 0) {
    if (!failureLog.includes("## Candangolândia")) {
      failureLog += "\n## Candangolândia (Revisão de Enquadramento GPS / Sem Cobertura)\n";
    }
    noCoverageHydrants.forEach(nom => {
      if (!failureLog.includes(`- ${nom}`)) {
        failureLog += `- ${nom} (Sem cobertura Street View)\n`;
      }
    });
    failedHydrants.forEach(nom => {
      if (!failureLog.includes(`- ${nom}`)) {
        failureLog += `- ${nom} (Capturado via Fallback GPS)\n`;
      }
    });
  ...
  console.log(`\n==========================================================`);
  console.log(`🎉 EXTRAÇÃO DE CANDANGOLÂNDIA CONCLUÍDA!`);
  console.log(`   💾 Registros atualizados na base: ${countUpdated}`);
  if (noCoverageHydrants.length > 0) {
    console.log(`⚠️ HIDRANTES SEM COBERTURA STREET VIEW:`);
    noCoverageHydrants.forEach(nom => console.log(` - ${nom}`));
  }
  if (failedHydrants.length > 0) {
    console.log(`ℹ️ HIDRANTES COM FALLBACK GPS (Salvos em rascunho):`);
