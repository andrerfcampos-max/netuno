...
import { isHydrantInSet } from './idMapping';

/**
 * Valida de forma uniforme se o hidrante está contido em uma lista de IDs (selecionados ou concluídos),
 * cruzando codHidrante, nomHidrante, _internalId e equivalência canônica para prevenir problemas de dessincronização.
 * @param {Object} hydrant O objeto do hidrante
 * @param {Array<string>|Set<string>} selectedIds A lista de IDs da missão
 * @returns {boolean}
 */
export const isHydrantSelected = (hydrant, selectedIds) => {
  return isHydrantInSet(hydrant, selectedIds);
};
