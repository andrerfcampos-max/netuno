...
import { calculateDistance, optimizeClosedLoopTSP } from '../utils/geoUtils';

// Refinamento e Telemetria Viária OSRM em Segundo Plano (Mapeia distâncias e tempos reais por via de trânsito)
const fetchOSRMInitialChunk = async (chunkHydrants, startLat, startLng) => {
  ...
  try {
    ...
    // Os chunkHydrants já estão ordenados pelo Closed-Loop TSP com anéis viários contínuos!
    // Mapeia as métricas viárias reais ao longo da rota ordenada:
    // Nó 0: Partida (startLat, startLng / Localização do Usuário)
    // Nó 1: Alvo 1 (chunkHydrants[0]) -> distances[0][1] (distância real da residência/partida até o 1º hidrante)
    // Nós subsequentes: trecho do hidrante anterior para o próximo (distances[i][i+1])
    for (let i = 0; i < chunkHydrants.length; i++) {
      const targetHydrant = chunkHydrants[i];
      const fromMatrixIdx = i;
      const toMatrixIdx = i + 1;
      
      const distMeters = (distances[fromMatrixIdx] && distances[fromMatrixIdx][toMatrixIdx] !== null && distances[fromMatrixIdx][toMatrixIdx] !== undefined) 
        ? distances[fromMatrixIdx][toMatrixIdx] 
        : null;
      const durSec = (durations[fromMatrixIdx] && durations[fromMatrixIdx][toMatrixIdx] !== null && durations[fromMatrixIdx][toMatrixIdx] !== undefined) 
        ? durations[fromMatrixIdx][toMatrixIdx] 
        : null;
    ...
    return { route: chunkHydrants, drivingMetrics, isTrafficMode: true };
  } catch (error) {
...
const MissionRoutePanel = ({ 
  ...
  const lastOptimizedSignatureRef = useRef('');
  const lastStartCoordRef = useRef(null);
  ...
  // Algoritmo Híbrido de Alta Performance (Closed-Loop TSP com 2-Opt + Telemetria OSRM)
  useEffect(() => {
    if (pendingHydrants.length === 0) {
      ...
      lastOptimizedSignatureRef.current = '';
      lastStartCoordRef.current = null;
      return;
    }

    const startLat = userLocation?.lat !== undefined ? userLocation.lat : pendingHydrants[0].numLatitude;
    const startLng = userLocation?.lng !== undefined ? userLocation.lng : pendingHydrants[0].numLongitude;

    const pendingSignature = pendingHydrants.map(h => h.codHidrante || h._internalId || h.nomHidrante).join(',');
    
    // Se a lista de hidrantes não mudou e o deslocamento do ponto de partida for pequeno (< 1km),
    // preservamos a rota otimizada e evitamos sobrescritas desnecessárias pelo GPS.
    const lastStart = lastStartCoordRef.current;
    const distMoved = lastStart ? calculateDistance(lastStart.lat, lastStart.lng, startLat, startLng) : 999;

    if (lastOptimizedSignatureRef.current === pendingSignature && distMoved < 1.0) {
      return;
    }

    // 1. ORDENAÇÃO INSTANTÂNEA CLOSED-LOOP TSP (com 2-Opt e anel viário contínuo)
    const optimalTour = optimizeClosedLoopTSP(pendingHydrants, startLat, startLng);
    setPendingRoute(optimalTour);
    lastStartCoordRef.current = { lat: startLat, lng: startLng };
    lastOptimizedSignatureRef.current = pendingSignature;

    // Métricas geodésicas instantâneas iniciais
    const initialMetrics = {};
    let prevLat = startLat;
    let prevLng = startLng;
    optimalTour.forEach((h, idx) => {
      ...
      if (k1) initialMetrics[k1] = metric;
      if (k2) initialMetrics[k2] = metric;
      if (k3) initialMetrics[k3] = metric;
      prevLat = h.numLatitude;
      prevLng = h.numLongitude;
    });
    setDrivingMetrics(initialMetrics);

    // 2. MICRO-TELEMETRIA VIÁRIA OSRM EM BACKGROUND (Apenas para o lote inicial de até 15 hidrantes)
    let isCancelled = false;
    const refineWithOSRM = async () => {
      setIsOptimizing(true);
      const CHUNK_LIMIT = 15;
      const immediateBatch = optimalTour.slice(0, CHUNK_LIMIT);

      const osrmResult = await fetchOSRMInitialChunk(immediateBatch, startLat, startLng);
      
      if (!isCancelled) {
        if (osrmResult.isTrafficMode && osrmResult.drivingMetrics) {
          setDrivingMetrics(prev => ({ ...prev, ...osrmResult.drivingMetrics }));
    ...
    const timer = setTimeout(refineWithOSRM, 300);
    return () => {
      isCancelled = true;
      clearTimeout(timer);
    };
  ...
  const handleRecalculateRoute = () => {
    ...
    setPendingRoute(optimalTour);
    lastStartCoordRef.current = { lat: startLat, lng: startLng };
    lastOptimizedSignatureRef.current = pendingHydrants.map(h => h.codHidrante || h._internalId || h.nomHidrante).join(',');
    setIsOptimizing(true);
  ...
  const renderHydrantItem = (h, index, isCompleted) => {
    ...
    // Para o 1º lugar (Alvo Imediato): a distância reflete a distância real da posição do usuário até o hidrante
    if (!isCompleted && index === 0) {
      ...
      // Para os demais alvos (2º em diante): reflete a distância do trecho a partir do alvo anterior
      if (metric && metric.distanceMeters !== null && metric.distanceMeters !== undefined) {
        const dMeters = metric.distanceMeters;
        const dText = dMeters < 1000 ? `${Math.round(dMeters)}m` : `${(dMeters / 1000).toFixed(1)}km`;
        const durMin = metric.durationSeconds ? `~${Math.max(1, Math.ceil(metric.durationSeconds / 60))}min` : '';
        distText = durMin ? `${dText} • ${durMin}` : dText;
      } else if (index > 0 && pendingRoute[index - 1]) {
        const prevH = pendingRoute[index - 1];
        const dist = calculateDistance(prevH.numLatitude, prevH.numLongitude, h.numLatitude, h.numLongitude);
        const dText = dist < 1 ? `${Math.round(dist * 1000)}m` : `${dist.toFixed(1)}km`;
        distText = dText;
      } else if (userLocation) {
        const dist = calculateDistance(userLocation.lat, userLocation.lng, h.numLatitude, h.numLongitude);
    ...
    if (!isCompleted && index === 0) {
      return (
        ...
        >
          ...
          <div className="flex items-center justify-between gap-1.5 w-full">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                ...
                {h.dscLocalidade && (
                  <span 
                    className="bg-slate-700/90 border border-slate-600/70 text-cyan-300 text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm shrink-0 truncate max-w-[120px]"
          ...
          {/* LINHA 2: TELEMETRIA TÁTICA DE PRÓXIMO ALVO (DISTÂNCIA E TEMPO INTEGRAL) */}
          <div className="flex items-center justify-between gap-1.5 bg-emerald-950/40 border border-emerald-500/40 px-2.5 py-1 rounded-lg">
            <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 text-[10px] font-black px-1.5 py-0.5 rounded-full shrink-0 flex items-center gap-1">
              <span>🎯</span> Próximo Alvo
            </span>
            {distText && (
              <span className="text-[11px] font-mono font-bold text-emerald-300 flex items-center gap-1 shrink-0">
                <span>📍</span> {distText}
              </span>
            )}
          </div>

          {/* LINHA 3: ENDEREÇO, REFERÊNCIA E ALERTAS */}
    ...
    // ========================================================
    // DEMAIS LUGARES (2º, 3º, etc. e Concluídos): ESTRUTURA TÁTICA DE 2 LINHAS
    // ========================================================
    let itemClasses = "w-full rounded-xl p-2 sm:p-2.5 flex flex-col gap-1.5 transition-all overflow-hidden border-l-4 ";
    if (isCompleted) {
    ...
    return (
      <div key={h._internalId || h.codHidrante || h.nomHidrante || index} className={itemClasses}>
        {/* LINHA 1: IDENTIFICAÇÃO (ESQUERDA) + TELEMETRIA/DISTÂNCIA INTEGRAL (DIREITA) */}
        <div className="flex items-center justify-between gap-1.5 w-full">
          <div className="flex items-center gap-1.5 min-w-0 flex-1">
            {/* Sequência / Número */}
            <div className={
              isCompleted 
                ? "bg-slate-700 text-slate-400 font-bold text-[11px] rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center shrink-0" 
                : "bg-emerald-950 text-emerald-400 border border-emerald-500/30 font-bold text-[11px] rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center shrink-0"
            }>
              {isCompleted ? "✓" : index + 1}
            </div>
            ...
            {!isCompleted && h.fotoPerfil && (
              <img src={h.fotoPerfil} alt="Perfil" className="w-6 h-6 object-cover rounded-md border border-slate-600 shrink-0 cursor-pointer hover:scale-105 transition-transform" />
            )}
            ...
            {h.dscLocalidade && (
              <span 
                className="bg-slate-700/80 border border-slate-600/60 text-cyan-300 text-[9px] font-bold px-1 py-0.5 rounded shrink-0 truncate max-w-[90px]"
                title={fixEncoding(h.dscLocalidade)}
              >
                {getShortRaName(h.dscLocalidade)}
              </span>
          ...
          {/* Distância e Tempo Sem Truncamento na Direita */}
          {distText && (
            <span className="text-[9.5px] sm:text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border flex items-center gap-1 shrink-0 bg-slate-850 border-slate-700 text-emerald-300 shadow-sm whitespace-nowrap">
              <span>📍</span> {distText}
            </span>
          )}
        </div>

        {/* LINHA 2: ENDEREÇO (ESQUERDA) + BOTÕES DE AÇÃO (DIREITA) */}
        <div className="flex items-center justify-between gap-1.5 w-full pt-0.5">
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-slate-400 text-[10px] sm:text-[11px] truncate leading-tight" title={h.dscEndereco || ''}>
              {fixEncoding(h.dscEndereco) || 'Endereço não informado'}
            </span>
            {h.dscPontoReferencia && (
              <span className="text-slate-500 italic text-[9px] truncate" title={h.dscPontoReferencia}>
                Ref: {fixEncoding(h.dscPontoReferencia)}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {!isCompleted && (
              <button 
                ...
                className="h-6 px-1.5 sm:px-2 bg-blue-600/90 hover:bg-blue-600 text-white rounded-md text-[10px] active:scale-95 transition-all flex items-center gap-1 font-semibold shadow-sm cursor-pointer"
              >
                <Navigation size={11} />
                <span className="hidden sm:inline text-[9px]">Waze</span>
              </button>
            )}

            {/* Botão Editar (Apenas Gestor/Admin) */}
            {(!isCompleted && (currentUser?.role === 'gestor' || currentUser?.role === 'admin')) && (
              <button 
                onClick={() => onEdit && onEdit(h)} 
                title="Editar Hidrante" 
                className="h-6 w-6 flex items-center justify-center bg-amber-700/90 hover:bg-amber-600 text-white rounded-md active:scale-95 transition-all shadow-sm cursor-pointer"
              >
                <Edit size={11}/>
              </button>
            )}
            ...
            {!isCompleted && (
              <button 
                onClick={() => onInspect && onInspect(h)} 
                title="Cadastrar Vistoria Técnica" 
                className="h-6 px-2 text-white rounded-md active:scale-95 transition-all font-bold text-[10px] flex items-center gap-1 shadow-md cursor-pointer bg-emerald-600 hover:bg-emerald-500 shrink-0"
              >
            ...
            {/* Botão Excluir / Remover da Missão (Se Gestor/Criador) */}
            {canEditRoute && (
              <button 
                onClick={() => handleRemoveItem(h)} 
                title="Remover da Missão" 
                className="h-6 w-6 flex items-center justify-center bg-rose-950/80 text-rose-400 hover:bg-rose-800 hover:text-white border border-rose-800/40 rounded-md active:scale-95 transition-all cursor-pointer"
              >
                <X size={11}/>
              </button>
            )}
        ...
        {/* ALERTA DE DEFEITO (Se existir) */}
        {h.problemasHidrante && h.problemasHidrante.trim() !== '' && (
          <div className="inline-flex items-center gap-1 bg-amber-950/60 border border-amber-600/40 text-amber-300 text-[9px] font-bold px-1.5 py-0.5 rounded truncate w-fit mt-0.5">
            <AlertTriangle size={11} className="shrink-0 text-amber-400" />
            <span>{fixEncoding(sanitizeProblem(h.problemasHidrante))}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex flex-col p-2 sm:p-3 w-full h-full bg-slate-900 relative overflow-hidden">
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-3 p-6 text-center">
            ...
            <p className="text-sm font-semibold text-slate-300 max-w-sm">
              {currentUser?.role === 'vistoriador'
                ? "Para cumprir a missão de vistoria do seu quartel, clique na Central de Missões"
                : "Para criar uma nova rota de missão, selecione os hidrantes no mapa ou na lista."}
            </p>
            {currentUser?.role === 'vistoriador' && onBackToManager && (
              <button
                type="button"
                onClick={onBackToManager}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-lg transition-all active:scale-95 cursor-pointer mt-1"
              >
                Ir para a Central de Missões
              </button>
            )}
          </div>
        ) : (
      ...
      <div className={`grid ${onGenerateReport ? 'grid-cols-2' : 'grid-cols-1'} gap-2 pb-0.5 pt-1.5 border-t border-slate-700/80 relative flex-shrink-0`}>
        <button 
