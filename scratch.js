...
const FilterBar = ({ activeFilters, onFilterChange, regions, anos = [], problemasAtivos = [], isVisible, currentUser, onLogout, filteredCount = null, onDrawerOpen, onDrawerClose }) => {
  ...
  const [isMobileDrawerOpen, setDrawerOpen] = useState(false);
  const setDrawerOpen = (val) => { setDrawerOpen(val); if (val && onDrawerOpen) onDrawerOpen(); if (!val && onDrawerClose) onDrawerClose(); };
